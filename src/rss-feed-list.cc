...
static const int XML_PARSE_FLAGS =
    XML_PARSE_NOBLANKS | XML_PARSE_NOERROR | XML_PARSE_NOWARNING;

void RSSFeedList::parse() noexcept(false) {
  xmlDocPtr doc =
      xmlReadFile(url.c_str(), /* encoding = */ NULL, XML_PARSE_FLAGS);
  if (doc == NULL) {
    // This is the only real user error we handle with any frequency, as it's
    // completely reasonable that the client more than occasionally specifies a
    // bogus URL.
  ...
  for (int i = 0; i < numItems; i++) {
    ...
    xmlChar *title =
        (titles->nodesetval == NULL || titles->nodesetval->nodeNr == 0)
            ? xmlCharStrdup("")
            : xmlNodeGetContent(titles->nodesetval->nodeTab[0]);
    ...
    xmlChar *link =
        (links->nodesetval == NULL || links->nodesetval->nodeNr == 0)
            ? xmlCharStrdup("")
            : xmlNodeGetContent(links->nodesetval->nodeTab[0]);
