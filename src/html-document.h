...
public:
   ...
   * ---------------------------------
   * Returns a const reference to the encapsulated URL (expressed as a C++
   * string).
   */
...
private:
   ...
   * otherwise get for the copy constructor and operator=.  Because the
   * implementation of parse() involves networking code, it's not clear what the
   * semantics of a deep copy really should be.  By deleting these two items, we
   * force all clients of the HTMLDocument class to pass instances around by
   * reference or by address.
   */
